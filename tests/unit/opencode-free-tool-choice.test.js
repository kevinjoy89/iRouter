...
describe("opencode Free 1.3 tool_choice auto-only", () => {
  ...
  it.each([
    ...
    for (const model of [FREE_13, `${FREE_13}(max)`]) {
      ...
      expect(out.tools).toEqual(expect.arrayContaining(TOOLS));
  ...
  it("giữ auto và absent; tools/input nguyên vẹn", () => {
    ...
    expect(autoOut.tools).toEqual(expect.arrayContaining(TOOLS));
    expect(autoOut.tools.length).toBe(5);
    ...
    expect(absentOut.tools).toEqual(expect.arrayContaining(TOOLS));
    expect(absentOut.tools.length).toBe(5);
  ...
  it("wire: execute gửi choice auto tới /zen/v1/responses", async () => {
    ...
    expect(sent.tools).toEqual(expect.arrayContaining(TOOLS));
    expect(sent.tools.length).toBe(5);
