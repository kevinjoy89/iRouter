...
const upstream = createServer((req, res) => {
  ...
  req.on("end", () => {
    upstreamHits.push({ url: req.url, auth: req.headers.authorization, body: body.slice(0, 400) });
    if (req.url.includes("/chat/completions")) {
      const wantsStream = /"stream"\s*:\s*true/.test(body);
      if (wantsStream) {
        res.writeHead(200, { "Content-Type": "text/event-stream", "Cache-Control": "no-cache", Connection: "keep-alive" });
        const chunk = (o) => res.write(`data: ${JSON.stringify(o)}\n\n`);
        chunk({ id: "chatcmpl-mock", object: "chat.completion.chunk", created: Math.floor(Date.now() / 1000),
                model: "mock-model", choices: [{ index: 0, delta: { role: "assistant", content: "MOCK_OK" }, finish_reason: null }] });
        chunk({ id: "chatcmpl-mock", object: "chat.completion.chunk", created: Math.floor(Date.now() / 1000),
...
console.log(`[gw]   starting gateway on 127.0.0.1:${port}, DATA_DIR=${dataDir}`);

const gw = spawn(process.execPath, [join(STANDALONE, "custom-server.js"), "--port", String(port)], {
  cwd: STANDALONE,
  env: { ...process.env, PORT: String(port), HOSTNAME: "127.0.0.1", DATA_DIR: dataDir,
         JWT_SECRET: "verify-secret", INITIAL_PASSWORD: "verify-pass-123", NODE_ENV: "production" },
...
for (;;) {
  ...
  if (Date.now() > deadline) { console.log("gateway never came up:\n" + gwLog.slice(-2000)); process.exit(1); }
...
try {
  ...
  check("4.7 cookie 带 24h maxAge (86400)",
    !!maxAge && Number(maxAge[1]) === 86400,
    maxAge ? `max-age=${maxAge[1]}` : `no max-age in: ${cookieStr.slice(0, 160)}`);

  const authToken = /auth_token=([^;]+)/.exec(cookieStr)?.[1];

  // ---------- 4.9 /v1/messages through the claude->openai bridge ----------
  const hdr = { "Content-Type": "application/json", ...(authToken ? { Cookie: `auth_token=${authToken}` } : {}) };

  // a) gateway API key (required to call /v1/*)
  const keyRes = await fetch(`${base}/api/keys`, { method: "POST", headers: hdr, body: JSON.stringify({ name: "verify" }) });
  const keyJson = await keyRes.json().catch(() => ({}));
  const gwKey = keyJson.key;
  check("4.9 创建网关 API key", !!gwKey, `status=${keyRes.status} ${JSON.stringify(keyJson).slice(0, 120)}`);

  // b) custom OpenAI-compatible node pointing at the mock upstream
  const nodeRes = await fetch(`${base}/api/provider-nodes`, {
    method: "POST", headers: hdr,
    body: JSON.stringify({ name: "MockUpstream", prefix: "mockup", apiType: "chat",
                           baseUrl: `http://127.0.0.1:${upstreamPort}/v1`, type: "openai-compatible" }),
  });
  const nodeJson = await nodeRes.json().catch(() => ({}));
  const nodeId = nodeJson.node?.id || nodeJson.id;
  check("4.9 注册 OpenAI 兼容节点", nodeRes.status < 400 && !!nodeId,
  ...
  const connRes = await fetch(`${base}/api/providers`, {
    method: "POST", headers: hdr,
    body: JSON.stringify({ provider: nodeId, apiKey: "sk-mock-upstream", name: "mock", priority: 1 }),
  });
  const connJson = await connRes.json().catch(() => ({}));
  check("4.9 添加连接", connRes.status < 400,
  ...
  const msgRes = await fetch(`${base}/v1/messages`, {
    method: "POST",
    headers: { "Content-Type": "application/json", "x-api-key": gwKey || "sk-none" },
    body: JSON.stringify({
  ...
  check("4.9 /v1/messages 请求成功", msgRes.status === 200, `status=${msgRes.status} body=${msgText.slice(0, 220)}`);
  check("4.9 收到上游回复内容", /MOCK_OK/.test(msgText), msgText.slice(0, 160));
  check("4.9 上游确实被调用 (claude→openai 翻译生效)", upstreamHits.length > 0,
    upstreamHits.length ? `hits=${upstreamHits.length} path=${upstreamHits[0].url}` : "no upstream hit");
  if (upstreamHits.length) {
    const u = upstreamHits[0];
    let names = [], body = {};
    try { body = JSON.parse(u.body); names = (body.messages || []).map((m) => m.role); } catch {}
    // system 提示被前置，故为 system + 3 轮对话；裸对象 content 那一轮必须在场
    check("4.9 裸对象 content 未导致该轮丢失 (上游收到 4 条含 assistant)",
      names.join(",") === "system,user,assistant,user" && /prior turn/.test(u.body),
      `roles=${JSON.stringify(names)} hasPriorTurn=${/prior turn/.test(u.body)}`);
  }
  // 响应诊断：非流式下的 content-type 与长度
  console.log(`[diag] /v1/messages status=${msgRes.status} ctype=${msgRes.headers.get("content-type")} len=${msgText.length}`);
  console.log(`[diag] body head: ${JSON.stringify(msgText.slice(0, 300))}`);
  // 完整落盘，便于人工核对 MOCK_OK 确实经 claude 事件流透传
  const { writeFileSync } = await import("node:fs");
  writeFileSync("/tmp/msg-stream.txt", msgText);
  ...
  check("4.8 定价接口不包含 gemini_weekly/claude_gpt_weekly",
    !/gemini_weekly|claude_gpt_weekly/.test(pricingText), `status=${pricing.status}`);

  // ---------- logout clears the cookie ----------
  const logout = await fetch(`${base}/api/auth/logout`, {
    method: "POST", headers: authToken ? { Cookie: `auth_token=${authToken}` } : {},
  });
  const logoutCookie = ((logout.headers.getSetCookie?.() ?? [logout.headers.get("set-cookie")]) || []).filter(Boolean).join("; ");
  check("4.7 登出清除 cookie", /auth_token=;|auth_token="";|max-age=0/i.test(logoutCookie),
