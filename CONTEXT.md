...
**请求脱敏（request redaction）**:
网关把请求转发给上游供应商之前，按规则检测并改写请求体中敏感内容的能力。三档动作：仅告警（audit）、改写后转发（redact）、拦截不转发（block）。默认关闭。威胁模型是"信任本机、不信任上游"——防的是凭据随 prompt 或工具输出外泄。
_Avoid_: DLP（易被理解为含端点/网络/分类分级/审计报表的完整体系，本功能只覆盖出站请求体这一段）
...
正文中一对固定标记围出的区间，声明"这段确实要发"。启用后该区间跳过检测，标记在转发前剥除；未配对或嵌套的标记按普通正文处理。默认关闭（可信单用户部署的逃生舱，不是访问控制）。
