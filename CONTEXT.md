...
iRouter 是 9Router（开源 AI 路由网关，decolua/9router）的跨平台安装版：装完是一个真正的桌面应用，窗口内直接显示 9Router 面板，无需打开系统浏览器。9Router 源码基于上游 **v0.5.86** 定制，位于本仓库根目录（`src/`、`open-sse/`、`tests/` 等）；桌面壳层在 `desktop/`。
...
**定制基线（v0.5.86 baseline）**:
9Router 源码基于上游 decolua/9router v0.5.86 定制，位于仓库根目录，可自由修改；升级 = 对比上游新版本手工合并本地定制。原 submodule 锁版结构已废弃（ADR 0002/0003 为历史决策）。
...
iRouter 自身对外可见的版本号（当前 `0.3.0`），真源为 `desktop/package.json`，构建期经 `NEXT_PUBLIC_APP_VERSION` 注入面板。与**上游基线号**（根 `package.json`，当前 `0.5.86`）解耦：后者只用于 UA / `X-Msh-Version` / `_meta.appVersion` 等与上游对齐的标识，不外露。二者不同不是缺陷，勿统一（ADR 0004）。
...
**网关数据目录（gateway data dir）**:
存放网关的配置、数据库、密钥与壳层设置文件的位置，由 `DATA_DIR` 环境变量决定。未设置时上游默认 `~/.9router`；**iRouter 桌面版默认 `~/.irouter`**。首次运行可一键导入旧 CLI 的 `~/.9router` 数据。
与**应用数据目录**不是同一个地方（见下条）：只有这里装数据库。
...
把网关的**配置层**整体导出为一个 JSON 文件、或从这样一个文件导入替换的动作。覆盖设置、供应商连接（含凭据）、供应商节点、代理池、API key、组合模型、模型别名/自定义模型/pricing 等若干类。
**不含**用量与请求日志——导入替换的是配置层，不是回到某个时间点。入口在壳层设置模态框的「Gateway data」段（桌面专属，见 ADR 0006）。
_Avoid_: 备份/还原（会被理解为完整回滚，且与下条撞名）

**schema 安全备份（schema safety backup）**:
schema 迁移**前**自动做的 SQLite 文件快照，保留最新三份，排除体积巨大的 `requestDetails` 表，**无自动恢复路径**——恢复靠手工把文件拷回去。它是迁移的兜底，用户不可见。
与「配置导出/导入」是两回事：一个是自动的整库快照（只增不减、无回滚），一个是手动的配置层搬运（可回滚配置、不动历史）。
_Avoid_: 备份（单独用会与配置导出混淆）
