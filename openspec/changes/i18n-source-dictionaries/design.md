...
- 面板 i18n 为运行时 DOM 替换方案：`src/i18n/runtime.js` 按 cookie 取 locale，fetch `/i18n/literals/{locale}.json` 得到 `{英文整句: 译文}` 映射，MutationObserver 遍历文本节点精确替换；`en` 不加载字典；`<select>` 子树与属性（title 等）不翻译
...
4. **title 属性不翻**：运行时不处理属性，保持简洁英文；用户可见的主要说明一律走文本节点（可翻译）。
