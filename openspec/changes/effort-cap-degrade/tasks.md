...
- [x] 1.1 ADR 0003 取代 ADR 0002 零改动条款：`9router/` 直接改源码、自维护（升级 = 切 tag 后合并改动）；CONTEXT.md 同步更新"锁版上游"词条并新增 组合模型/思考强度上限/主动钳制/反应式降级重试/能力感知排序 术语；验证：docs/adr/0003-self-maintained-upstream.md 存在，CONTEXT.md 术语与其他文件一致

## 2. 核心逻辑

- [x] 2.1 新增 `9router/open-sse/services/effortCaps.js`：`getDeclaredLevels`（声明集合解析，剥思考后缀）/ `clampLevel`（钳到集合边界）/ `resolveRequestedEffort`（后缀优先于 body 字段，识别 4 种 body 形状）/ `applyEffortToBody` / `nextLowerLevel`（按声明集合跳档）/ `isInvalidEffortError`（窄匹配 400/422）；验证：tests/unit/effort-caps.test.js 22 用例通过
