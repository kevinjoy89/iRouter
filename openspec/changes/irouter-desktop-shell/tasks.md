...
- [x] 2.4 托盘与自启：菜单含打开面板 / 开机自启 checkbox / 退出；自启默认关闭，开启后写入登录项且重启保持，自启启动（wasOpenedAtLogin）时不弹窗驻留托盘；已验证：开机自启行为正常，开关及驻留托盘正常
...
- [x] 3.3 dmg 人工安装验证（spec 验收）：安装后启动即见面板（无 Node 依赖）、数据写入平台目录、导入的旧配置可用、关窗驻留托盘、托盘退出无残留；已人工验收通过
