...
```text
desktop/build/dist/
├── iRouter-0.1.8.dmg            <-- [发布交付] macOS 磁盘镜像安装包（约 149 MB）
├── iRouter-0.1.8.dmg.blockmap   <-- 块更新校验文件
...
1. 双击打开 `iRouter-0.1.8.dmg`，将 `iRouter.app` 拖入 `/Applications`（应用程序目录）；
...
- [ ] **品牌与版本展示**：侧边栏与个人设置页（`/dashboard/profile`）底部均展示为 `iRouter Proxy v0.1.8`；
